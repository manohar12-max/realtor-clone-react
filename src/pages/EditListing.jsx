...
const CreateListing = () => {
  ...
  const onChange = (e) => {
    ...
    if (!e.target.files) {
      setFormData((prevState) => ({
        ...prevState, //if boolean is null then take e.target.value else take boolean
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    //we have added + to convert it to number tpo string
    ...
    if (geolocationEnabled) {
      // use api from google cloud its paid
    } else {
  ...
  //   check if edit listing belongs to the person who is editing it
useEffect(()=>{
    
    if(listing && listing.userRef !==  auth.currentUser.uid){
        toast.error("You are not authorized edit this listing")
        navigate("/")
    }   
